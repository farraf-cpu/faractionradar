"""Personal Income predictor + emitter. `v1-simple-blend`.

Monthly release, ~last business day of month, 08:30 ET by BEA. Same
day/time as PCE Price Index and Personal Spending — the BEA income +
outlays trio. Personal Income m/m %-change (nominal): wages + salaries
+ transfers + rents + interest + dividends. Fed watches the
income-vs-spending gap for the household savings pulse.

Sub-models:
  - Bloomberg / FF consensus (~0.15pp MAE)
  - FRED PI 3-mo trend (~0.20pp MAE)

Value format: `+0.3%` m/m.

Env: UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL, FRED_API_KEY,
     INCOME_RELEASE_DATE, INCOME_DAYS_OUT, INCOME_CONSENSUS,
     MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-income] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_trend() -> float | None:
    """3-mo mean of FRED PI m/m %-change (nominal Personal Income)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=PI&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print(f"[emit-income] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def regime_annotation(value: float) -> str:
    if value >= 0.6:  return "strong income growth"
    if value >= 0.3:  return "healthy income growth"
    if value >= 0.0:  return "flat income growth"
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-income")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# Personal Income prediction — target {release} (T-{days_out})
...
**{format_value(point)}** m/m Personal Income
...
- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** — first ship. 30th event covered; completes BEA income + outlays trio (Income + Spending + PCE Price).
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-income] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-income] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "INCOME_RELEASE_DATE", "INCOME_DAYS_OUT"):
        require_env(k)

    release = os.environ["INCOME_RELEASE_DATE"]
    days_out = int(os.environ["INCOME_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("INCOME_CONSENSUS")
    trend = fetch_fred_trend()

    if consensus is None and trend is None:
        print("[emit-income] all sub-models missing; nothing to blend — exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, trend)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("income")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-income] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-income] Income {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"income-{release}",
        "eventTitle": "US Personal Income m/m",
        "country": "USD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/income-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"income-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-income] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-income] appended predictions.jsonl")
