"""UK CPI predictor. v1-simple-blend.

Publishes UK CPI y/y %-change point estimate. ONS releases CPI for
reference month ~mid-month of following month at 07:00 UK time.

Value format: y/y %-change (e.g. "+2.5%").
Sub-models:
  - FF consensus (~0.10pp MAE)
  - FRED CPALTT01GBM659N 3-mo mean y/y anchor (~0.20pp MAE)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     UKCPI_RELEASE_DATE, UKCPI_DAYS_OUT, UKCPI_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-ukcpi] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_trend() -> float | None:
    """3-mo mean of FRED CPALTT01GBM659N (UK CPI y/y %-change)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=CPALTT01GBM659N&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print(f"[emit-ukcpi] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def regime_annotation(value: float) -> str:
    if value >= 4.0:  return "hot UK inflation (BoE hawkish pressure)"
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-ukcpi")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# UK CPI prediction - target {release} (T-{days_out})
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-ukcpi] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-ukcpi] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "UKCPI_RELEASE_DATE", "UKCPI_DAYS_OUT"):
        require_env(k)

    release = os.environ["UKCPI_RELEASE_DATE"]
    days_out = int(os.environ["UKCPI_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("UKCPI_CONSENSUS")
    trend = fetch_fred_trend()

    if consensus is None and trend is None:
        print("[emit-ukcpi] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, trend)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("ukcpi")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-ukcpi] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-ukcpi] UKCPI {release} T-{days_out}: {format_value(point)} y/y "
    ...
    prediction = {
        "eventSlug": f"ukcpi-{release}",
        "eventTitle": "UK CPI y/y",
        "country": "GBP",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/ukcpi-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"ukcpi-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-ukcpi] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-ukcpi] appended predictions.jsonl")
