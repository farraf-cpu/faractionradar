"""BOJ Policy Rate predictor. v2-outcome-distribution.

Bank of Japan monetary policy meeting decisions (~8x/year). Post-2024
exit from negative rates, BOJ moves in 15-25bp increments. Publishes
implied Policy Rate as point estimate PLUS 25bp outcome distribution.

Value format: rate level (e.g. "0.75%"). Sub-models:
  - FF consensus (~0.05pp MAE - analysts converge on likely move)
  - FRED IRSTCI01JPM156N current-rate anchor (~0.15pp MAE - OECD
    Immediate Rates <24h for Japan, tracks BOJ policy rate within
    ~5-10bp)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     BOJ_RELEASE_DATE, BOJ_DAYS_OUT, BOJ_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-boj] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current BOJ Policy Rate proxy from FRED IRSTCI01JPM156N (OECD
    Immediate Rates <24h for Japan). Updates monthly and tracks BOJ
    policy decisions within ~5-10bp."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=IRSTCI01JPM156N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-boj] FRED IRSTCI01JPM156N fetch failed: {e}", file=sys.stderr)
        return None
...
def compute_outcome_distribution(point: float, sigma: float,
    ...
    """v2 outcome-distribution over BOJ 25bp rate outcomes. Post-2024
    exit from NIRP, BOJ moves in 15-25bp steps; we use 25bp buckets
    consistent with FOMC/ECB/BOE v2."""
    if anchor is None:
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-boj")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# BOJ Policy Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** BOJ Policy Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 4 JPY expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-boj] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-boj] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "BOJ_RELEASE_DATE", "BOJ_DAYS_OUT"):
        require_env(k)

    release = os.environ["BOJ_RELEASE_DATE"]
    days_out = int(os.environ["BOJ_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("BOJ_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-boj] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("boj")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-boj] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_anchor(point, anchor)
    outcome_dist = compute_outcome_distribution(point, sigma, anchor)

    print(f"[emit-boj] BOJ {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"boj-{release}",
        "eventTitle": "BOJ Policy Rate",
        "country": "JPY",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/boj-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"boj-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-boj] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-boj] appended predictions.jsonl")
