...
Value format: `+0.3%` m/m Core PPI (ex food + energy).
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-coreppi] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def regime_annotation(value: float) -> str:
    if value >= 0.4:  return "hot core wholesale inflation"
    if value >= 0.25: return "elevated core wholesale inflation"
    if value >= 0.15: return "on-target core wholesale inflation"
    return "cooling core wholesale inflation"
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-coreppi")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# Core PPI prediction — target {release} (T-{days_out})
...
**{format_value(point)}** m/m Core PPI (ex food + energy)
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-coreppi] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-coreppi] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "COREPPI_RELEASE_DATE", "COREPPI_DAYS_OUT"):
        require_env(k)
    ...
    if sigma_source.startswith("empirical"):
        print(f"[emit-coreppi] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    ...
    prediction = {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/coreppi-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"coreppi-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-coreppi] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-coreppi] appended predictions.jsonl")
