"""UK Monthly GDP predictor. v1-simple-blend.

ONS publishes monthly UK GDP m/m %-change ~40 days after reference month
at 07:00 UK time. Monthly GDP is not on FRED cleanly - consensus-only
...
Env: UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     UKGDP_RELEASE_DATE, UKGDP_DAYS_OUT, UKGDP_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-ukgdp] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-ukgdp")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# UK Monthly GDP prediction - target {release} (T-{days_out})
...
- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Third Phase 3 GBP predictor. Consensus-only pending ONS API integration.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-ukgdp] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-ukgdp] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "UKGDP_RELEASE_DATE", "UKGDP_DAYS_OUT"):
        require_env(k)

    release = os.environ["UKGDP_RELEASE_DATE"]
    days_out = int(os.environ["UKGDP_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("UKGDP_CONSENSUS")

    if consensus is None:
        print("[emit-ukgdp] consensus missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("ukgdp")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-ukgdp] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-ukgdp] UKGDP {release} T-{days_out}: {format_value(point)} m/m "
          f"(sigma {sigma:.2f}pp, {regime_annotation(point)}, used: {', '.join(used)})")
    if consensus is not None: print(f"  consensus: {consensus:+.2f}%")

    prediction = {
        "eventSlug": f"ukgdp-{release}",
        "eventTitle": "UK Monthly GDP m/m",
        "country": "GBP",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/ukgdp-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"ukgdp-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-ukgdp] wrote {report_path.relative_to(ROOT)}")
