"""Factory Orders predictor. `v1-simple-blend`.

Monthly release, ~first week of month, 10:00 ET by Census (BEA M3
Survey full report). Follows the advance Durable Goods print (which
FF/Bloomberg release ~last week of prior month) by ~5-7 business days.
The full report adds nondurable orders + revised durables + inventories.

Value format: `+0.5%` m/m of total manufacturers' new orders.

Sub-models:
  - Bloomberg / FF consensus (~0.3pp MAE)
  - FRED AMTMNO 3-mo mean m/m trend (~0.5pp MAE — series is volatile
    on aircraft-order cycles)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     FACTORY_RELEASE_DATE, FACTORY_DAYS_OUT, FACTORY_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-factory] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_trend() -> float | None:
    """3-mo mean of FRED AMTMNO m/m %-change (Manufacturers' New Orders total)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=AMTMNO&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print(f"[emit-factory] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def regime_annotation(value: float) -> str:
    if value >= 1.5:  return "strong new-order expansion"
    if value >= 0:    return "modest expansion"
    if value >= -1.5: return "modest contraction"
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-factory")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# Factory Orders prediction — target {release} (T-{days_out})
...
**{format_value(point)}** m/m Factory Orders (total manufacturers' new orders)
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-factory] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-factory] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "FACTORY_RELEASE_DATE", "FACTORY_DAYS_OUT"):
        require_env(k)

    release = os.environ["FACTORY_RELEASE_DATE"]
    days_out = int(os.environ["FACTORY_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("FACTORY_CONSENSUS")
    trend = fetch_fred_trend()

    if consensus is None and trend is None:
        print("[emit-factory] all sub-models missing; nothing to blend — exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, trend)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("factory")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-factory] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-factory] Factory Orders {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"factory-{release}",
        "eventTitle": "US Factory Orders m/m",
        "country": "USD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/factory-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"factory-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-factory] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-factory] appended predictions.jsonl")
