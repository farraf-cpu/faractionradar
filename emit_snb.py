"""SNB Policy Rate predictor. v2-outcome-distribution.

Swiss National Bank Monetary Policy Report / Policy Rate decisions
(~4x/year (quarterly)). Decision at 09:45
Ottawa time (14:45 UTC winter / 13:45 UTC summer).

Value format: rate level (e.g. "2.25%"). Sub-models:
  - FF consensus (~0.05pp MAE)
  - FRED IR3TIB01CHM156N (3-mo CHF rate (SARON-adjacent)) current-rate
    anchor (~0.30pp MAE (term premium) - tracks SNB overnight rate within ~15-25bp (term premium noted))

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     SNB_RELEASE_DATE, SNB_DAYS_OUT, SNB_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-snb] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current SNB Policy Rate proxy from FRED IR3TIB01CHM156N
    (3-mo CHF rate (SARON-adjacent)). Tracks SNB overnight rate within
    ...
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=IR3TIB01CHM156N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-snb] FRED IR3TIB01CHM156N fetch failed: {e}", file=sys.stderr)
        return None
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-snb")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# SNB Policy Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** SNB Policy Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 8 CHF expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-snb] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-snb] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "SNB_RELEASE_DATE", "SNB_DAYS_OUT"):
        require_env(k)

    release = os.environ["SNB_RELEASE_DATE"]
    days_out = int(os.environ["SNB_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("SNB_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-snb] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("snb")
    ...
    prediction = {
        "eventSlug": f"snb-{release}",
        "eventTitle": "SNB Policy Rate",
        "country": "CHF",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/snb-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"snb-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-snb] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-snb] appended predictions.jsonl")
