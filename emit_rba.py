"""RBA Cash Rate predictor. v2-outcome-distribution.

Reserve Bank of Australia Monetary Policy Meeting decisions (~11x/year
post-2024 reform). Decision at 14:30 AEDT / 03:30 UTC (AEDT is UTC+11
in summer, UTC+10 winter).

Value format: rate level (e.g. "3.60%"). Sub-models:
  - FF consensus (~0.05pp MAE)
  - FRED IRSTCI01AUM156N (OECD Immediate Rates <24h AU) current-rate
    anchor (~0.15pp MAE - tracks RBA cash rate within ~5-10bp)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     RBA_RELEASE_DATE, RBA_DAYS_OUT, RBA_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-rba] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current RBA Cash Rate proxy from FRED IRSTCI01AUM156N (OECD
    Immediate Rates <24h AU). Tracks RBA cash rate within ~5-10bp,
    monthly updates. FRED INTDSRAUM193N (AU Discount Rate) is
    discontinued 2013 (Rule 23)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=IRSTCI01AUM156N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-rba] FRED IRSTCI01AUM156N fetch failed: {e}", file=sys.stderr)
        return None
...
def compute_outcome_distribution(point: float, sigma: float,
    ...
    """v2 outcome-distribution over standard RBA 25bp rate outcomes."""
    if anchor is None:
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-rba")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# RBA Cash Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** RBA Cash Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 5 AUD expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-rba] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-rba] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "RBA_RELEASE_DATE", "RBA_DAYS_OUT"):
        require_env(k)

    release = os.environ["RBA_RELEASE_DATE"]
    days_out = int(os.environ["RBA_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("RBA_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-rba] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("rba")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-rba] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_anchor(point, anchor)
    outcome_dist = compute_outcome_distribution(point, sigma, anchor)

    print(f"[emit-rba] RBA {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"rba-{release}",
        "eventTitle": "RBA Cash Rate",
        "country": "AUD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/rba-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"rba-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-rba] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-rba] appended predictions.jsonl")
