"""BOE Bank Rate predictor. v2-outcome-distribution.

Publishes implied Bank of England Bank Rate as point estimate PLUS
probability distribution over standard 25bp outcomes. MPC meets 8x/year;
decision at 12:00 UK time (11:00 or 12:00 UTC depending on BST).

Value format: rate level (e.g. "4.00%"). Sub-models:
  - FF consensus (~0.05pp MAE - analysts converge on likely move)
  - FRED IUDSOIA (SONIA overnight rate) current-rate anchor (~0.25pp MAE -
    tracks Bank Rate within ~5-10bp, assumes no change at meeting)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     BOE_RELEASE_DATE, BOE_DAYS_OUT, BOE_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-boe] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current BoE Bank Rate proxy from FRED IUDSOIA (SONIA overnight rate).
    SONIA tracks the Bank Rate within ~5-10bp and updates daily.
    FRED INTDSRGBM193N (UK Discount Rate) is discontinued (frozen at 0.5%
    since 2013) and unsuitable as an anchor."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=IUDSOIA&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-boe] FRED IUDSOIA fetch failed: {e}", file=sys.stderr)
        return None
...
def compute_outcome_distribution(point: float, sigma: float,
    ...
    """v2 outcome-distribution over standard BoE 25bp rate outcomes.
    Same discretization pattern as FOMC + ECB v2. Buckets relative to
    current Bank Rate anchor."""
    if anchor is None:
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-boe")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# BOE Bank Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** BoE Bank Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 3 GBP expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-boe] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-boe] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "BOE_RELEASE_DATE", "BOE_DAYS_OUT"):
        require_env(k)

    release = os.environ["BOE_RELEASE_DATE"]
    days_out = int(os.environ["BOE_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("BOE_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-boe] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("boe")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-boe] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_anchor(point, anchor)
    outcome_dist = compute_outcome_distribution(point, sigma, anchor)

    print(f"[emit-boe] BOE {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"boe-{release}",
        "eventTitle": "BOE Bank Rate",
        "country": "GBP",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/boe-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"boe-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-boe] wrote {report_path.relative_to(ROOT)}")
