...
IBGE publishes quarterly GDP q/q ~11 weeks
after quarter end at 10:45 BRLT (12:00 UTC). Consensus-only:
FRED CHNGDPNQDSMEI doesn't exist; IBGE Infoshare API integration
deferred to v1.1.
...
Env: UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     BRGDP_RELEASE_DATE, BRGDP_DAYS_OUT, BRGDP_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-brgdp] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-brgdp")
...
- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Third Phase 15 BRL predictor. Consensus-only pending IBGE API integration.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-brgdp] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-brgdp] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "BRGDP_RELEASE_DATE", "BRGDP_DAYS_OUT"):
        require_env(k)

    release = os.environ["BRGDP_RELEASE_DATE"]
    days_out = int(os.environ["BRGDP_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("BRGDP_CONSENSUS")

    if consensus is None:
        print("[emit-brgdp] consensus missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("brgdp")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-brgdp] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-brgdp] BRGDP {release} T-{days_out}: {format_value(point)} q/q "
          f"(sigma {sigma:.2f}pp, {regime_annotation(point)}, used: {', '.join(used)})")
    if consensus is not None: print(f"  consensus: {consensus:+.2f}%")

    prediction = {
        "eventSlug": f"brgdp-{release}",
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/brgdp-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"brgdp-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-brgdp] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-brgdp] appended predictions.jsonl")
