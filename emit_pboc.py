"""PBOC 1Y LPR predictor. v2-outcome-distribution.

People's Bank of China 1-Year Loan Prime Rate (LPR) fixed monthly
on ~20th at 09:15 CST (01:15 UTC). Primary bank-lending benchmark
since 2019 LPR reform.

Value format: rate level (e.g. "3.10%"). Sub-models:
  - FF consensus (~0.05pp MAE)
  - FRED INTDSRCNM193N (PBOC Discount Rate, closest live proxy)
    anchor (~0.30pp MAE - somewhat stale but tracks policy stance
    within ~20bp of LPR)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     PBOC_RELEASE_DATE, PBOC_DAYS_OUT, PBOC_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-pboc] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current PBOC Policy Rate proxy from FRED INTDSRCNM193N
    (3-mo CNY rate (SARON-adjacent)). Tracks PBOC overnight rate within
    ...
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=INTDSRCNM193N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-pboc] FRED INTDSRCNM193N fetch failed: {e}", file=sys.stderr)
        return None
...
def compute_outcome_distribution(point: float, sigma: float,
    ...
    """v2 outcome-distribution over standard PBOC 25bp rate outcomes."""
    if anchor is None:
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-pboc")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# PBOC Policy Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** PBOC Policy Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 10 CNY expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-pboc] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-pboc] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "PBOC_RELEASE_DATE", "PBOC_DAYS_OUT"):
        require_env(k)

    release = os.environ["PBOC_RELEASE_DATE"]
    days_out = int(os.environ["PBOC_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("PBOC_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-pboc] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("pboc")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-pboc] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_anchor(point, anchor)
    outcome_dist = compute_outcome_distribution(point, sigma, anchor)

    print(f"[emit-pboc] PBOC {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"pboc-{release}",
        "eventTitle": "PBOC Policy Rate",
        "country": "CNY",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/pboc-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"pboc-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-pboc] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-pboc] appended predictions.jsonl")
