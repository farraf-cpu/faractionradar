"""BANXICO Policy Rate predictor. v2-outcome-distribution.

Banco de Mexico (Banxico) Monetary Policy Report / Policy Rate decisions
(~8x/year). Decision at 09:45
Ottawa time (19:00 UTC winter).

Value format: rate level (e.g. "2.25%"). Sub-models:
  - FF consensus (~0.05pp MAE)
  - FRED IRSTCI01MXM156N (OECD Immediate Rates <24h MX) current-rate
    anchor (~0.15pp MAE - tracks BANXICO overnight rate within ~5-10bp)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     BANXICO_RELEASE_DATE, BANXICO_DAYS_OUT, BANXICO_CONSENSUS, MODEL_VERSION
"""
...
MAE = {
    "consensus": 0.05,
    "anchor":    1.00,   # FRED IRSTCI01MXM156N is 100-200bp below actual Banxico target; heavily deweighted
}
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-banxico] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current BANXICO Policy Rate proxy from FRED IRSTCI01MXM156N
    (OECD Immediate Rates <24h MX). Tracks BANXICO overnight rate within
    ...
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=IRSTCI01MXM156N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-banxico] FRED IRSTCI01MXM156N fetch failed: {e}", file=sys.stderr)
        return None
...
def compute_outcome_distribution(point: float, sigma: float,
    ...
    """v2 outcome-distribution over standard BANXICO 25bp rate outcomes."""
    if anchor is None:
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-banxico")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# BANXICO Policy Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** BANXICO Policy Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 13 MXN expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-banxico] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-banxico] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "BANXICO_RELEASE_DATE", "BANXICO_DAYS_OUT"):
        require_env(k)

    release = os.environ["BANXICO_RELEASE_DATE"]
    days_out = int(os.environ["BANXICO_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("BANXICO_CONSENSUS")
    anchor = fetch_fred_anchor()

    # Guard: FRED IRSTCI01MXM156N runs ~100-200bp below actual Banxico
    ...
    if consensus is not None and anchor is not None and abs(consensus - anchor) > 0.75:
        print(f"[emit-banxico] anchor {anchor:.2f}% vs consensus {consensus:.2f}% > 75bp; dropping anchor")
        anchor = None

    if consensus is None and anchor is None:
        print("[emit-banxico] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("banxico")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-banxico] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    ...
    print(f"[emit-banxico] BANXICO {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"banxico-{release}",
        "eventTitle": "BANXICO Policy Rate",
        "country": "MXN",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/banxico-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"banxico-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-banxico] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-banxico] appended predictions.jsonl")
