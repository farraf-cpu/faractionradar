...
GUS publishes monthly CPI y/y
~1 week after reference month at 08:30 CET (07:30 UTC winter).
SARB targets 3-6% CPI y/y band (+/- 1pp).

Consensus-only for v1: FRED's CPALTT01PLM659N is stale (last obs
2025-03, usable but slow-moving). GUS Statistical Portal API
...
Env: UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     PLCPI_RELEASE_DATE, PLCPI_DAYS_OUT, PLCPI_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-plcpi] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-plcpi")
...
Second Phase 21 (PLN expansion) predictor. National Core CPI y/y is
RBNZ's preferred gauge. Released by GUS ~19th-27th of
following month at 10:45 PLNT.

## Caveats

FRED coverage for Japan CPI is dead — an GUS GUS Statistical Portal API integration
(stat.gov.pl, free with registration) would give a real trend
anchor. Phase 21.1 target.

## Change log

- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 21 PLN expansion. Consensus-only pending GUS GUS Statistical Portal API.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-plcpi] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-plcpi] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "PLCPI_RELEASE_DATE", "PLCPI_DAYS_OUT"):
        require_env(k)

    release = os.environ["PLCPI_RELEASE_DATE"]
    days_out = int(os.environ["PLCPI_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("PLCPI_CONSENSUS")

    if consensus is None:
        print("[emit-plcpi] consensus missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("plcpi")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-plcpi] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-plcpi] PLCPI {release} T-{days_out}: {format_value(point)} y/y "
          f"(sigma {sigma:.2f}pp, {regime_annotation(point)}, used: {', '.join(used)})")
    if consensus is not None: print(f"  consensus: {consensus:+.2f}%")

    prediction = {
        "eventSlug": f"plcpi-{release}",
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/plcpi-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"plcpi-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-plcpi] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-plcpi] appended predictions.jsonl")
