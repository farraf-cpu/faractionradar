"""Wholesale Inventories predictor. `v1-simple-blend`.

Monthly release, ~day 8-10 of month, 10:00 ET by Census (Monthly
Wholesale Trade: Sales and Inventories). ~1 week before the full
Business Inventories report — wholesale is the leading sector for
inventory-cycle detection (mfg absorbs demand shocks first).

Value format: `+0.2%` m/m of merchant wholesalers inventories.

Sub-models:
  - Bloomberg / FF consensus (~0.15pp MAE)
  - FRED WHLSLRIMSA 3-mo mean m/m trend (~0.25pp MAE)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     WHOLESALE_RELEASE_DATE, WHOLESALE_DAYS_OUT,
     WHOLESALE_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-wholesale] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_trend() -> float | None:
    """3-mo mean of FRED WHLSLRIMSA m/m %-change."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=WHLSLRIMSA&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print(f"[emit-wholesale] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def regime_annotation(value: float) -> str:
    if value >= 0.5:  return "wholesale buildup"
    if value >= 0:    return "modest wholesale growth"
    if value >= -0.3: return "wholesale drawdown"
    return "sharp wholesale liquidation"
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-wholesale")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# Wholesale Inventories prediction — target {release} (T-{days_out})
...
**{format_value(point)}** m/m Wholesale Inventories
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-wholesale] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-wholesale] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "WHOLESALE_RELEASE_DATE", "WHOLESALE_DAYS_OUT"):
        require_env(k)

    release = os.environ["WHOLESALE_RELEASE_DATE"]
    days_out = int(os.environ["WHOLESALE_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("WHOLESALE_CONSENSUS")
    trend = fetch_fred_trend()

    if consensus is None and trend is None:
        print("[emit-wholesale] all sub-models missing; nothing to blend — exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, trend)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("wholesale")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-wholesale] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-wholesale] Wholesale {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"wholesale-{release}",
        "eventTitle": "US Wholesale Inventories m/m",
        "country": "USD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/wholesale-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"wholesale-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-wholesale] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-wholesale] appended predictions.jsonl")
