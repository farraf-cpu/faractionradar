"""BCCH Policy Rate predictor. v2-outcome-distribution.

Banco Central de Chile Monetary Policy Report / Policy Rate decisions
(~8x/year (monthly)). Decision at 09:45
Ottawa time (13:30 UTC).

Value format: rate level (e.g. "2.25%"). Sub-models:
  - FF consensus (~0.05pp MAE)
  - FRED IRSTCI01CLM156N (OECD Immediate Rates <24h CL) current-rate
    anchor (~0.15pp MAE - tracks BCCH overnight rate within ~5-10bp)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     BCCH_RELEASE_DATE, BCCH_DAYS_OUT, BCCH_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-bcch] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current BCCH Policy Rate proxy from FRED IRSTCI01CLM156N
    (OECD Immediate Rates <24h CL). Tracks BCCH overnight rate within
    ...
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=IRSTCI01CLM156N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-bcch] FRED IRSTCI01CLM156N fetch failed: {e}", file=sys.stderr)
        return None
...
def compute_outcome_distribution(point: float, sigma: float,
    ...
    """v2 outcome-distribution over standard BCCH 25bp rate outcomes."""
    if anchor is None:
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-bcch")
...
**{format_value(point)}** BCCH Policy Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 22 CLP expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-bcch] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-bcch] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "BCCH_RELEASE_DATE", "BCCH_DAYS_OUT"):
        require_env(k)

    release = os.environ["BCCH_RELEASE_DATE"]
    days_out = int(os.environ["BCCH_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("BCCH_CONSENSUS")
    anchor = fetch_fred_anchor()

    # Rule 35: FRED IRSTCI01CLM156N may report POLONIA overnight
    # (~3.7%) which differs from BCCH TPM (Monetary Policy Rate) (~5-6%). Drop
    # anchor when |consensus - anchor| > 75bp.
    if consensus is not None and anchor is not None and abs(consensus - anchor) > 0.75:
        print(f"[emit-bcch] anchor {anchor:.2f}% vs consensus {consensus:.2f}% > 75bp; dropping anchor")
        anchor = None

    if consensus is None and anchor is None:
        print("[emit-bcch] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("bcch")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-bcch] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    ...
    print(f"[emit-bcch] BCCH {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"bcch-{release}",
        "eventTitle": "BCCH Policy Rate",
        "country": "CLP",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/bcch-model-card.md",
    }
    ...
    print(f"[emit-bcch] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-bcch] appended predictions.jsonl")
