"""Philly Fed Manufacturing Index predictor + emitter. `v1-simple-blend`.

Monthly release, ~3rd Thursday of month, 08:30 ET by Federal Reserve Bank
of Philadelphia. Diffusion index where 0 = neutral (same scale as Empire).
Second regional Fed survey each month after Empire State.

Sub-models:
  - Bloomberg / FF consensus (~4 index points MAE)
  - Last-known anchor (~6 pts MAE)

No FRED trend in v1 — Philly Fed's headline series ID is not consistently
available on FRED (employment sub-index NECDFSA066MSFRBPHI is available
but tracks a different thing). Add real trend in Phase 2 once verified.

Env: UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     PHILLY_RELEASE_DATE, PHILLY_DAYS_OUT, PHILLY_CONSENSUS,
     PHILLY_ANCHOR, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-philly] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-philly")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# Philly Fed Manufacturing prediction — target {release} (T-{days_out})
...
**{format_value(point)}** Philly Fed General Business Activity
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-philly] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-philly] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "PHILLY_RELEASE_DATE", "PHILLY_DAYS_OUT"):
        require_env(k)

    release = os.environ["PHILLY_RELEASE_DATE"]
    days_out = int(os.environ["PHILLY_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("PHILLY_CONSENSUS")
    anchor = parse_float("PHILLY_ANCHOR")

    if consensus is None and anchor is None:
        print("[emit-philly] all sub-models missing; nothing to blend — exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("philly")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-philly] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-philly] Philly {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"philly-{release}",
        "eventTitle": "US Philly Fed Manufacturing Index",
        "country": "USD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/philly-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"philly-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-philly] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-philly] appended predictions.jsonl")
