"""NORGES Policy Rate predictor. v2-outcome-distribution.

Norges Bank Monetary Policy Report / Policy Rate decisions
(~8x/year). Decision at 09:45
Ottawa time (09:00 UTC).

Value format: rate level (e.g. "2.25%"). Sub-models:
  - FF consensus (~0.05pp MAE)
  - FRED IRSTCI01NOM156N (OECD Immediate Rates <24h NO) current-rate
    anchor (~0.15pp MAE - tracks NORGES overnight rate within ~5-10bp)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     NORGES_RELEASE_DATE, NORGES_DAYS_OUT, NORGES_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-norges] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current NORGES Policy Rate proxy from FRED IRSTCI01NOM156N
    (OECD Immediate Rates <24h NO). Tracks NORGES overnight rate within
    ...
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=IRSTCI01NOM156N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-norges] FRED IRSTCI01NOM156N fetch failed: {e}", file=sys.stderr)
        return None
...
def compute_outcome_distribution(point: float, sigma: float,
    ...
    """v2 outcome-distribution over standard NORGES 25bp rate outcomes."""
    if anchor is None:
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-norges")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# NORGES Policy Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** NORGES Policy Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 11 NOK expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-norges] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-norges] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "NORGES_RELEASE_DATE", "NORGES_DAYS_OUT"):
        require_env(k)

    release = os.environ["NORGES_RELEASE_DATE"]
    days_out = int(os.environ["NORGES_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("NORGES_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-norges] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("norges")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-norges] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_anchor(point, anchor)
    outcome_dist = compute_outcome_distribution(point, sigma, anchor)

    print(f"[emit-norges] NORGES {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"norges-{release}",
        "eventTitle": "NORGES Policy Rate",
        "country": "NOK",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
    ...
    report_path = ROOT / "reports" / year_month / f"norges-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-norges] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-norges] appended predictions.jsonl")
