"""RBNZ Official Cash Rate predictor. v2-outcome-distribution.

Reserve Bank of New Zealand Monetary Policy Report / Official Cash Rate decisions
(~7x/year (post-2022 reform)). Decision at 09:45
Ottawa time (14:45 UTC winter / 13:45 UTC summer).

Value format: rate level (e.g. "2.25%"). Sub-models:
  - FF consensus (~0.05pp MAE)
  - FRED IR3TIB01NZM156N (3-mo Interbank Rate NZ) current-rate
    anchor (~0.30pp MAE (term premium) - tracks RBNZ overnight rate within ~15-25bp (term premium noted))

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     RBNZ_RELEASE_DATE, RBNZ_DAYS_OUT, RBNZ_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-rbnz] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current RBNZ Official Cash Rate proxy from FRED IR3TIB01NZM156N
    (3-mo Interbank Rate NZ). Tracks RBNZ overnight rate within
    ...
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=IR3TIB01NZM156N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-rbnz] FRED IR3TIB01NZM156N fetch failed: {e}", file=sys.stderr)
        return None
...
def compute_outcome_distribution(point: float, sigma: float,
    ...
    """v2 outcome-distribution over standard RBNZ 25bp rate outcomes."""
    if anchor is None:
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-rbnz")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# RBNZ Official Cash Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** RBNZ Official Cash Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 7 NZD expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-rbnz] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-rbnz] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "RBNZ_RELEASE_DATE", "RBNZ_DAYS_OUT"):
        require_env(k)

    release = os.environ["RBNZ_RELEASE_DATE"]
    days_out = int(os.environ["RBNZ_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("RBNZ_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-rbnz] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("rbnz")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-rbnz] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_anchor(point, anchor)
    outcome_dist = compute_outcome_distribution(point, sigma, anchor)

    print(f"[emit-rbnz] RBNZ {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"rbnz-{release}",
        "eventTitle": "RBNZ Official Cash Rate",
        "country": "NZD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/rbnz-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"rbnz-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-rbnz] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-rbnz] appended predictions.jsonl")
