"""NB Policy Rate predictor. v2-outcome-distribution.

Danmarks Nationalbank Monetary Policy Report / Policy Rate decisions
(~8x/year). Decision at 09:45
Ottawa time (13:30 UTC).

Value format: rate level (e.g. "2.25%"). Sub-models:
  - FF consensus (~0.05pp MAE)
  - FRED IRSTCI01DKM156N (OECD Immediate Rates <24h DK) current-rate
    anchor (~0.15pp MAE - tracks NB overnight rate within ~5-10bp)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     NB_RELEASE_DATE, NB_DAYS_OUT, NB_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-nb] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current NB Policy Rate proxy from FRED IRSTCI01DKM156N
    (OECD Immediate Rates <24h DK). Tracks NB overnight rate within
    ...
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=IRSTCI01DKM156N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-nb] FRED IRSTCI01DKM156N fetch failed: {e}", file=sys.stderr)
        return None
...
def compute_outcome_distribution(point: float, sigma: float,
    ...
    """v2 outcome-distribution over standard NB 25bp rate outcomes."""
    if anchor is None:
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-nb")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# NB Policy Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** NB Policy Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 23 DKK expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-nb] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-nb] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "NB_RELEASE_DATE", "NB_DAYS_OUT"):
        require_env(k)

    release = os.environ["NB_RELEASE_DATE"]
    days_out = int(os.environ["NB_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("NB_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-nb] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("nb")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-nb] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_anchor(point, anchor)
    outcome_dist = compute_outcome_distribution(point, sigma, anchor)

    print(f"[emit-nb] NB {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"nb-{release}",
        "eventTitle": "NB Policy Rate",
        "country": "DKK",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/nb-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"nb-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-nb] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-nb] appended predictions.jsonl")
