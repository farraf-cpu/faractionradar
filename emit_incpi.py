...
MoSPI publishes monthly CPI y/y
~1 week after reference month at 08:30 CET (07:30 UTC winter).
RBI targets 4% CPI y/y (2-6% band) (+/- 1pp).

Consensus-only for v1: FRED's CPALTT01INM659N is stale (last obs
2025-03, usable but slow-moving). MoSPI Statistical Portal API
...
def main() -> None:
    ...
    if consensus is None:
        print("[emit-incpi] consensus missing; nothing to blend - exit 0 (soft skip)")
        return
