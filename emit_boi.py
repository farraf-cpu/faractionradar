"""BOI Policy Rate predictor. v2-outcome-distribution.

Bank of Israel Monetary Policy Report / Policy Rate decisions
(~8x/year). Decision at 09:45
Ottawa time (13:00 UTC).

Value format: rate level (e.g. "2.25%"). Sub-models:
  - FF consensus (~0.05pp MAE)
  - FRED IRSTCI01ILM156N (OECD Immediate Rates <24h IL) current-rate
    anchor (~0.15pp MAE - tracks BOI overnight rate within ~5-10bp)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     BOI_RELEASE_DATE, BOI_DAYS_OUT, BOI_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-boi] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current BOI Policy Rate proxy from FRED IRSTCI01ILM156N
    (OECD Immediate Rates <24h IL). Tracks BOI overnight rate within
    ...
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=IRSTCI01ILM156N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-boi] FRED IRSTCI01ILM156N fetch failed: {e}", file=sys.stderr)
        return None
...
def compute_outcome_distribution(point: float, sigma: float,
    ...
    """v2 outcome-distribution over standard BOI 25bp rate outcomes."""
    if anchor is None:
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-boi")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# BOI Policy Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** BOI Policy Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 18 ILS expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-boi] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-boi] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "BOI_RELEASE_DATE", "BOI_DAYS_OUT"):
        require_env(k)

    release = os.environ["BOI_RELEASE_DATE"]
    days_out = int(os.environ["BOI_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("BOI_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-boi] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("boi")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-boi] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_anchor(point, anchor)
    outcome_dist = compute_outcome_distribution(point, sigma, anchor)

    print(f"[emit-boi] BOI {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"boi-{release}",
        "eventTitle": "BOI Policy Rate",
        "country": "ILS",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/boi-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"boi-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-boi] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-boi] appended predictions.jsonl")
