"""BCB 1Y Selic predictor. v2-outcome-distribution.

Banco Central do Brasil Selic Target Rate) fixed monthly
on ~20th at 18:30 BRT (21:30 UTC). Primary bank-lending benchmark
since 2019 Selic reform.

Value format: rate level (e.g. "3.10%"). Sub-models:
  - FF consensus (~0.05pp MAE)
  - FRED INTDSRBRM193N (BCB Discount Rate, closest live proxy)
    anchor (~0.30pp MAE - somewhat stale but tracks policy stance
    within ~20bp of Selic)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     BCB_RELEASE_DATE, BCB_DAYS_OUT, BCB_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-bcb] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Current BCB Policy Rate proxy from FRED INTDSRBRM193N
    (3-mo BRL rate (SARON-adjacent)). Tracks BCB overnight rate within
    ...
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           "series_id=INTDSRBRM193N&api_key=" + api_key + "&file_type=json"
    ...
    try:
        ...
        print(f"[emit-bcb] FRED INTDSRBRM193N fetch failed: {e}", file=sys.stderr)
        return None
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-bcb")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# BCB Policy Rate prediction - target {release} (T-{days_out})
...
**{format_value(point)}** BCB Policy Rate
...
- **v2-outcome-distribution ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 15 BRL expansion opens.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-bcb] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-bcb] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "BCB_RELEASE_DATE", "BCB_DAYS_OUT"):
        require_env(k)

    release = os.environ["BCB_RELEASE_DATE"]
    days_out = int(os.environ["BCB_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v2-outcome-distribution")

    consensus = parse_float("BCB_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-bcb] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("bcb")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-bcb] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_anchor(point, anchor)
    outcome_dist = compute_rate_outcome_distribution(point, sigma, anchor, bucket_bp=50)

    print(f"[emit-bcb] BCB {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"bcb-{release}",
        "eventTitle": "BCB Policy Rate",
        "country": "BRL",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/bcb-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"bcb-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-bcb] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-bcb] appended predictions.jsonl")
