...
Stats SA publishes quarterly GDP q/q ~11 weeks
after quarter end at 10:45 ZART (12:00 UTC). Consensus-only:
FRED CHNGDPNQDSMEI doesn't exist; Stats SA Infoshare API integration
deferred to v1.1.
...
Env: UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     ZAGDP_RELEASE_DATE, ZAGDP_DAYS_OUT, ZAGDP_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-zagdp] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-zagdp")
...
`v1-simple-blend`: consensus-only (Stats SA monthly GDP is not on FRED
cleanly; Stats SA API integration deferred to v1.1). Soft-skips when FF
consensus missing.

## Positioning

Third Phase 16 (ZAR expansion) predictor. NZ GDP released by
Stats SA ~16 days after quarter end (early Stats SA release) at 10:45 ZART (12:00 UTC). Sits alongside
BoE Bank Rate + CA CPI in Phase 16 ZAR trio.
...
is the only reliable signal for v1. Phase 16.1 target: integrate Stats SA
`statssa.gov.za` timeseries endpoint for a real trend anchor.

## Change log

- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Third Phase 16 ZAR predictor. Consensus-only pending Stats SA API integration.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-zagdp] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-zagdp] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "ZAGDP_RELEASE_DATE", "ZAGDP_DAYS_OUT"):
        require_env(k)

    release = os.environ["ZAGDP_RELEASE_DATE"]
    days_out = int(os.environ["ZAGDP_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("ZAGDP_CONSENSUS")

    if consensus is None:
        print("[emit-zagdp] consensus missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("zagdp")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-zagdp] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-zagdp] ZAGDP {release} T-{days_out}: {format_value(point)} q/q "
          f"(sigma {sigma:.2f}pp, {regime_annotation(point)}, used: {', '.join(used)})")
    if consensus is not None: print(f"  consensus: {consensus:+.2f}%")

    prediction = {
        "eventSlug": f"zagdp-{release}",
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/zagdp-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"zagdp-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-zagdp] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-zagdp] appended predictions.jsonl")
