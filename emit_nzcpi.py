"""NZ CPI predictor. v1-simple-blend.

Statistics New Zealand (StatsNZ) publishes quarterly CPI y/y ~1 month
after quarter end at 10:45 NZDT (21:45 UTC prior day). Big trader
event on NZD; RBNZ targets 2% CPI y/y (1-3% band).

Consensus-only for v1: FRED's NZ CPI series CPALTT01NZQ659N is
discontinued (last obs 2023). StatsNZ Infoshare API integration
deferred to v1.1.
...
Env: UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     NZCPI_RELEASE_DATE, NZCPI_DAYS_OUT, NZCPI_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-nzcpi] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-nzcpi")
...
- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 7 NZD expansion. Consensus-only pending StatsNZ Infoshare API.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-nzcpi] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-nzcpi] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "NZCPI_RELEASE_DATE", "NZCPI_DAYS_OUT"):
        require_env(k)

    release = os.environ["NZCPI_RELEASE_DATE"]
    days_out = int(os.environ["NZCPI_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("NZCPI_CONSENSUS")

    if consensus is None:
        print("[emit-nzcpi] consensus missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("nzcpi")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-nzcpi] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-nzcpi] NZCPI {release} T-{days_out}: {format_value(point)} y/y "
          f"(sigma {sigma:.2f}pp, {regime_annotation(point)}, used: {', '.join(used)})")
    if consensus is not None: print(f"  consensus: {consensus:+.2f}%")

    prediction = {
        "eventSlug": f"nzcpi-{release}",
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/nzcpi-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"nzcpi-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-nzcpi] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-nzcpi] appended predictions.jsonl")
