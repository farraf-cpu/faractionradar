...
Statistics Sweden (SCB) publishes monthly CPI y/y
~1 week after reference month at 08:30 CET (07:30 UTC winter).
Riksbank targets 2% CPI y/y with tolerance band.

Consensus-only for v1: FRED's CPALTT01SEM659N is stale (last obs
2025-03, usable but slow-moving). SCB Statistical Portal API
...
Env: UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     SECPI_RELEASE_DATE, SECPI_DAYS_OUT, SECPI_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-secpi] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-secpi")
...
- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 9 SEK expansion. Consensus-only pending SCB SCB Statistical Portal API.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-secpi] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-secpi] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "SECPI_RELEASE_DATE", "SECPI_DAYS_OUT"):
        require_env(k)

    release = os.environ["SECPI_RELEASE_DATE"]
    days_out = int(os.environ["SECPI_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("SECPI_CONSENSUS")

    if consensus is None:
        print("[emit-secpi] consensus missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("secpi")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-secpi] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-secpi] SECPI {release} T-{days_out}: {format_value(point)} y/y "
          f"(sigma {sigma:.2f}pp, {regime_annotation(point)}, used: {', '.join(used)})")
    if consensus is not None: print(f"  consensus: {consensus:+.2f}%")

    prediction = {
        "eventSlug": f"secpi-{release}",
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/secpi-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"secpi-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-secpi] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-secpi] appended predictions.jsonl")
