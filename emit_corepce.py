...
MAE = {
    "consensus":     0.05,   # PCE consensus tighter than CPI (Fed target = more scrutiny)
    "cleveland_fed": 0.04,   # tightest sub-model - daily Cleveland Fed nowcast
    "trend":         0.10,
}
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    parts_tbl = "\n".join(
        f"| {name} | {'â€"' if v is None else f'{v:+.2f}%'} | {MAE[name]:.2f}pp |"
        for name, v in (("consensus", consensus), ("cleveland_fed", cleveland_fed), ("trend", trend))
    )
    return f"""# Core PCE prediction â€" target {release} (T-{days_out})
...
those to show underlying inflation trend. Sticky-Fed indicator â€"
prints >0.3% m/m sustain hawkish pressure; <0.2% opens easing path.

## Change log

- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** â€" first ship.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-corepce] worker â†' {res.status} {res.reason}")
...
def main() -> None:
    ...
    if consensus is None and cleveland_fed is None and trend is None:
        print("[emit-corepce] all sub-models missing; nothing to blend â€" exit 0 (soft skip)")
        return
