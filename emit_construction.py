"""Construction Spending predictor. `v1-simple-blend`.

Monthly release, 1st business day of month, 10:00 ET by Census.
2-month data lag (September release covers July data). Nominal
outlays across residential + nonresidential + public construction.

Value format: `+0.5%` m/m of total construction spending.

Sub-models:
  - Bloomberg / FF consensus (~0.3pp MAE)
  - FRED TTLCONS 3-mo mean m/m trend (~0.4pp MAE)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     CONSTRUCTION_RELEASE_DATE, CONSTRUCTION_DAYS_OUT,
     CONSTRUCTION_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-construction] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_trend() -> float | None:
    """3-mo mean of FRED TTLCONS m/m %-change (Total Construction Spending)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=TTLCONS&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print(f"[emit-construction] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def regime_annotation(value: float) -> str:
    if value >= 1.0:  return "strong construction expansion"
    if value >= 0:    return "modest expansion"
    if value >= -1.0: return "modest contraction"
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-construction")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# Construction Spending prediction — target {release} (T-{days_out})
...
**{format_value(point)}** m/m Construction Spending (total, nominal)
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-construction] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-construction] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "CONSTRUCTION_RELEASE_DATE", "CONSTRUCTION_DAYS_OUT"):
        require_env(k)

    release = os.environ["CONSTRUCTION_RELEASE_DATE"]
    days_out = int(os.environ["CONSTRUCTION_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("CONSTRUCTION_CONSENSUS")
    trend = fetch_fred_trend()

    if consensus is None and trend is None:
        print("[emit-construction] all sub-models missing; nothing to blend — exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, trend)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("construction")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-construction] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-construction] Construction {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"construction-{release}",
        "eventTitle": "US Construction Spending m/m",
        "country": "USD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/construction-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"construction-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-construction] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-construction] appended predictions.jsonl")
