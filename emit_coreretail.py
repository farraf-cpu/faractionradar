"""Core Retail Sales predictor + emitter. `v1-simple-blend`.

Core Retail Sales (ex food + energy) is the Fed's actual inflation focus, not
headline. Released same day/time as CPI headline (~mid-month, 08:30 ET,
BLS). FF publishes as separate event.

Value format: `+0.3%` m/m Core Retail Sales (ex food + energy).

Sub-models:
  - Bloomberg / FF consensus (~0.08pp MAE)
  - FRED RSFSXMV 6-mo mean m/m trend (~0.15pp MAE)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     CORERETAIL_RELEASE_DATE, CORERETAIL_DAYS_OUT, CORERETAIL_CONSENSUS,
     MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-coreretail] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_trend() -> float | None:
    """6-mo mean of FRED RSFSXMV m/m %-change (Core Retail Sales ex food+energy)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=RSFSXMV&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print(f"[emit-coreretail] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def regime_annotation(value: float) -> str:
    if value >= 0.4:  return "strong consumer spending"
    if value >= 0.25: return "healthy consumer spending"
    if value >= 0.15: return "modest consumer spending"
    return "consumer contraction"
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-coreretail")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# Core Retail Sales prediction — target {release} (T-{days_out})
...
**{format_value(point)}** m/m Core Retail Sales (ex food + energy)
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-coreretail] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-coreretail] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "CORERETAIL_RELEASE_DATE", "CORERETAIL_DAYS_OUT"):
        require_env(k)

    release = os.environ["CORERETAIL_RELEASE_DATE"]
    days_out = int(os.environ["CORERETAIL_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("CORERETAIL_CONSENSUS")
    trend = fetch_fred_trend()

    if consensus is None and trend is None:
        print("[emit-coreretail] all sub-models missing; nothing to blend — exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, trend)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("coreretail")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-coreretail] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-coreretail] Core Retail Sales {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"coreretail-{release}",
        "eventTitle": "US Core Retail Sales m/m",
        "country": "USD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/coreretail-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"coreretail-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-coreretail] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-coreretail] appended predictions.jsonl")
