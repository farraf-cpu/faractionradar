...
Statistics Sweden (SCB) publishes quarterly GDP q/q ~9 weeks
after quarter end at 09:30 CEST (07:30 UTC summer). Big trader event on SEK.

Value format: q/q %-change (e.g. "+0.5%").
Sub-models:
  - FF consensus (~0.15pp MAE)
  - FRED CLVMNACSCAB1GQSE 4-qtr q/q trend (~0.30pp MAE)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     SEGDP_RELEASE_DATE, SEGDP_DAYS_OUT, SEGDP_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-segdp] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_trend() -> float | None:
    """4-qtr mean q/q %-change of FRED CLVMNACSCAB1GQSE (Real GDP AU)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=CLVMNACSCAB1GQSE&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print(f"[emit-segdp] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-segdp")
...
- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 9 SEK expansion.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-segdp] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-segdp] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "SEGDP_RELEASE_DATE", "SEGDP_DAYS_OUT"):
        require_env(k)

    release = os.environ["SEGDP_RELEASE_DATE"]
    days_out = int(os.environ["SEGDP_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("SEGDP_CONSENSUS")
    trend = fetch_fred_trend()

    if consensus is None and trend is None:
        print("[emit-segdp] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, trend)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("segdp")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-segdp] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-segdp] SEGDP {release} T-{days_out}: {format_value(point)} q/q "
    ...
    prediction = {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/segdp-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"segdp-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-segdp] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-segdp] appended predictions.jsonl")
