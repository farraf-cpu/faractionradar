"""JP Preliminary GDP q/q predictor. v1-simple-blend.

Cabinet Office publishes Preliminary GDP q/q ~45 days after quarter
end at 08:50 JST. Trader event around JPY. Sub-models:
  - FF consensus (~0.15pp MAE - primary signal)
  - FRED NGDPRSAXDCJPQ 4-qtr mean q/q trend (~0.30pp MAE)

Value format: q/q %-change (e.g. "+0.2%").

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     JPGDP_RELEASE_DATE, JPGDP_DAYS_OUT, JPGDP_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-jpgdp] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_trend() -> float | None:
    """4-qtr mean q/q %-change of FRED NGDPRSAXDCJPQ (Real GDP JP)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=NGDPRSAXDCJPQ&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print(f"[emit-jpgdp] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def regime_annotation(value: float) -> str:
    if value >= 0.7:  return "strong quarterly expansion"
    if value >= 0.2:  return "modest growth"
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-jpgdp")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# JP Preliminary GDP prediction - target {release} (T-{days_out})
...
**{format_value(point)}** q/q JP Preliminary GDP
...
- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 4 JPY expansion.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-jpgdp] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-jpgdp] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "JPGDP_RELEASE_DATE", "JPGDP_DAYS_OUT"):
        require_env(k)

    release = os.environ["JPGDP_RELEASE_DATE"]
    days_out = int(os.environ["JPGDP_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("JPGDP_CONSENSUS")
    trend = fetch_fred_trend()

    if consensus is None and trend is None:
        print("[emit-jpgdp] all sub-models missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, trend)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("jpgdp")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-jpgdp] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-jpgdp] JPGDP {release} T-{days_out}: {format_value(point)} q/q "
    ...
    prediction = {
        "eventSlug": f"jpgdp-{release}",
        "eventTitle": "JP Preliminary GDP q/q",
        "country": "JPY",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/jpgdp-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"jpgdp-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-jpgdp] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-jpgdp] appended predictions.jsonl")
