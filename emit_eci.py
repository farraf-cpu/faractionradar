"""Employment Cost Index predictor. `v1-simple-blend`.

Quarterly release, ~end of month after quarter close, 08:30 ET by BLS.
Total compensation (civilian) q/q %-change. This is the Fed's cleanest
wage-inflation read — captures wages + benefits, avoids composition
bias that plagues Average Hourly Earnings from NFP.

Sub-models:
  - Bloomberg / FF consensus (~0.05pp MAE)
  - FRED ECIALLCIV last-print anchor (~0.10pp MAE — smooth persistence)

Value format: `+0.9%` q/q.

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     ECI_RELEASE_DATE, ECI_DAYS_OUT, ECI_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-eci] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_anchor() -> float | None:
    """Last-print q/q %-change of FRED ECIALLCIV (total civilian compensation)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=ECIALLCIV&api_key={api_key}&file_type=json"
           "&units=pch&sort_order=desc&limit=1")
    req = urllib.request.Request(url, headers={"user-agent": UA})
    try:
        with urllib.request.urlopen(req, timeout=20) as res:
            data = json.loads(res.read().decode("utf-8"))
    except Exception as e:
        print(f"[emit-eci] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def regime_annotation(value: float) -> str:
    if value >= 1.1:  return "hot wage growth (persistent inflation risk)"
    if value >= 0.8:  return "elevated wage growth"
    if value >= 0.5:  return "moderate wage growth"
    return "cooling wage growth"
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-eci")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# Employment Cost Index prediction — target {release} (T-{days_out})
...
**{format_value(point)}** q/q Employment Cost Index (total civilian compensation)
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-eci] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-eci] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "ECI_RELEASE_DATE", "ECI_DAYS_OUT"):
        require_env(k)

    release = os.environ["ECI_RELEASE_DATE"]
    days_out = int(os.environ["ECI_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("ECI_CONSENSUS")
    anchor = fetch_fred_anchor()

    if consensus is None and anchor is None:
        print("[emit-eci] all sub-models missing; nothing to blend — exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, anchor)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("eci")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-eci] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-eci] ECI {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"eci-{release}",
        "eventTitle": "US Employment Cost Index q/q",
        "country": "USD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/eci-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"eci-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-eci] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-eci] appended predictions.jsonl")
