...
INE publishes monthly CPI y/y
~1 week after reference month at 08:30 CET (07:30 UTC winter).
SARB targets 3-6% CPI y/y band (+/- 1pp).

Consensus-only for v1: FRED's CPALTT01CLM659N is stale (last obs
2025-03, usable but slow-moving). INE Statistical Portal API
...
Env: UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     CLCPI_RELEASE_DATE, CLCPI_DAYS_OUT, CLCPI_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-clcpi] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-clcpi")
...
- **v1-simple-blend ({datetime.now(timezone.utc).strftime('%Y-%m-%d')})** - first ship. Phase 22 CLP expansion. Consensus-only pending INE INE Statistical Portal API.
"""
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-clcpi] worker -> {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-clcpi] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "CLCPI_RELEASE_DATE", "CLCPI_DAYS_OUT"):
        require_env(k)

    release = os.environ["CLCPI_RELEASE_DATE"]
    days_out = int(os.environ["CLCPI_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("CLCPI_CONSENSUS")

    if consensus is None:
        print("[emit-clcpi] consensus missing; nothing to blend - exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("clcpi")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-clcpi] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-clcpi] CLCPI {release} T-{days_out}: {format_value(point)} y/y "
          f"(sigma {sigma:.2f}pp, {regime_annotation(point)}, used: {', '.join(used)})")
    if consensus is not None: print(f"  consensus: {consensus:+.2f}%")

    prediction = {
        "eventSlug": f"clcpi-{release}",
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/clcpi-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"clcpi-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-clcpi] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-clcpi] appended predictions.jsonl")
