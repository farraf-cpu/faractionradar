...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    _post_to_worker(url, auth_key, payload, tag="emit-krgdp")
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-krgdp")
...
def main() -> None:
    ...
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("krgdp")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-krgdp] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
