"""S&P/Case-Shiller 20-City Composite HPI predictor. `v1-simple-blend`.

Monthly release, last Tuesday of month, 09:00 ET by S&P Cotality
(formerly S&P Dow Jones Indices). Reports 2-month-lag data (e.g.
September release covers July data).

Value format: y/y %-change (e.g. "+2.1%"). Home-price appreciation
proxy — the "wealth-effect" input for consumption forecasts and a
lagging read on shelter-inflation direction.

Sub-models:
  - Bloomberg / FF consensus (~0.15pp MAE)
  - FRED SPCS20RSA 3-mo mean y/y (~0.25pp MAE — trend is smooth)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     CASESHILLER_RELEASE_DATE, CASESHILLER_DAYS_OUT,
     CASESHILLER_CONSENSUS, MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-caseshiller] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_trend() -> float | None:
    """3-mo mean of FRED SPCS20RSA y/y %-change (Case-Shiller 20-City SA)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=SPCS20RSA&api_key={api_key}&file_type=json"
           "&units=pc1&sort_order=desc&limit=3")
    req = urllib.request.Request(url, headers={"user-agent": UA})
    try:
        with urllib.request.urlopen(req, timeout=20) as res:
            data = json.loads(res.read().decode("utf-8"))
    except Exception as e:
        print(f"[emit-caseshiller] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def regime_annotation(value: float) -> str:
    if value >= 5:   return "strong home-price appreciation"
    if value >= 0:   return "moderate appreciation"
    if value >= -5:  return "mild decline"
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-caseshiller")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# S&P/Case-Shiller 20-City HPI prediction — target {release} (T-{days_out})
...
**{format_value(point)}** y/y S&P/Case-Shiller 20-City Composite HPI
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-caseshiller] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-caseshiller] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "CASESHILLER_RELEASE_DATE", "CASESHILLER_DAYS_OUT"):
        require_env(k)

    release = os.environ["CASESHILLER_RELEASE_DATE"]
    days_out = int(os.environ["CASESHILLER_DAYS_OUT"])
    model_version = os.environ.get("MODEL_VERSION", "v1-simple-blend")

    consensus = parse_float("CASESHILLER_CONSENSUS")
    trend = fetch_fred_trend()

    if consensus is None and trend is None:
        print("[emit-caseshiller] all sub-models missing; nothing to blend — exit 0 (soft skip)")
        return

    point, sigma, used = blend(consensus, trend)
    prior_sigma = sigma
    empirical_mae = fetch_empirical_mae("caseshiller")
    sigma, sigma_source = auto_tune_sigma(prior_sigma, empirical_mae)
    if sigma_source.startswith("empirical"):
        print(f"[emit-caseshiller] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    lean = lean_vs_consensus(point, consensus)

    print(f"[emit-caseshiller] Case-Shiller {release} T-{days_out}: {format_value(point)} "
    ...
    prediction = {
        "eventSlug": f"caseshiller-{release}",
        "eventTitle": "US S&P/Case-Shiller 20-City Composite HPI y/y",
        "country": "USD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/caseshiller-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"caseshiller-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-caseshiller] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-caseshiller] appended predictions.jsonl")
