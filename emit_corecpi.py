"""Core CPI predictor + emitter. `v1-simple-blend`.

Core CPI (ex food + energy) is the Fed's actual inflation focus, not
headline. Released same day/time as CPI headline (~mid-month, 08:30 ET,
BLS). FF publishes as separate event.

Value format: `+0.3%` m/m Core CPI (ex food + energy).

Sub-models:
  - Bloomberg / FF consensus (~0.08pp MAE)
  - FRED CPILFESL 6-mo mean m/m trend (~0.15pp MAE)

Env: FRED_API_KEY, UPLOAD_AUTH_KEY, CALENDAR_WORKER_URL,
     CORECPI_RELEASE_DATE, CORECPI_DAYS_OUT, CORECPI_CONSENSUS,
     MODEL_VERSION
"""
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-corecpi] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def fetch_fred_trend() -> float | None:
    """6-mo mean of FRED CPILFESL m/m %-change (Core CPI ex food+energy)."""
    api_key = os.environ.get("FRED_API_KEY")
    if not api_key:
        return None
    url = ("https://api.stlouisfed.org/fred/series/observations?"
           f"series_id=CPILFESL&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print(f"[emit-corecpi] FRED fetch failed: {e}", file=sys.stderr)
        return None
...
def fetch_cleveland_fed_nowcast() -> float | None:
    ...
    try:
        ...
        print(f"[emit-corecpi] Cleveland Fed fetch failed: {e}", file=sys.stderr)
        return None
    if not isinstance(data, list) or not data:
        return None
    for ds in data[0].get("dataset") or []:
        if ds.get("seriesname") != "Core CPI Inflation":
            continue
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-corecpi] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-corecpi] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "CORECPI_RELEASE_DATE", "CORECPI_DAYS_OUT"):
        require_env(k)

    release = os.environ["CORECPI_RELEASE_DATE"]
    days_out = int(os.environ["CORECPI_DAYS_OUT"])
    ...
    prediction = {
        "eventSlug": f"corecpi-{release}",
        "eventTitle": "US Core CPI m/m",
        "country": "USD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/corecpi-model-card.md",
    }
    ...
    print(f"[emit-corecpi] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-corecpi] appended predictions.jsonl")
