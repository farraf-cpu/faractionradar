...
single-plane Boeing orders can swing headline by 2pp+.
...
def require_env(key: str) -> str:
    ...
    if not v:
        print(f"[emit-durable] missing env: {key}", file=sys.stderr)
        sys.exit(2)
    return v
...
def lean_vs_consensus(point: float, consensus: float | None) -> str:
    ...
    if abs(delta) < 0.1:  # 10bp = noise on durables
...
def fetch_empirical_mae(slug_prefix: str) -> dict | None:
    return _fetch_empirical_mae(slug_prefix, tag="emit-durable")
...
def build_report_md(point: float, sigma: float, release: str, days_out: int,
    ...
    return f"""# Durable Goods Orders prediction — target {release} (T-{days_out})
...
**{format_value(point)} m/m** (Durable Goods Orders, headline)
...
def post_to_worker(url: str, auth_key: str, payload: dict) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as res:
            print(f"[emit-durable] worker → {res.status} {res.reason}")
    except urllib.error.HTTPError as e:
        print(f"[emit-durable] worker rejected: {e.code} {e.reason}", file=sys.stderr)
        print(e.read().decode("utf-8", errors="replace"), file=sys.stderr)
        sys.exit(3)


def main() -> None:
    for k in ("UPLOAD_AUTH_KEY", "CALENDAR_WORKER_URL", "DURABLE_RELEASE_DATE", "DURABLE_DAYS_OUT"):
        require_env(k)
    ...
    if sigma_source.startswith("empirical"):
        print(f"[emit-durable] sigma auto-tuned: prior={prior_sigma:.3f} -> empirical={sigma:.3f}")
    ...
    prediction = {
        ...
        "eventTitle": "US Durable Goods Orders m/m",
        "country": "USD",
        "releaseDate": release,
        "daysOut": days_out,
        "ourCall": {
        ...
        "grandMedian": None,
        "modelCardUrl": "https://github.com/farraf-cpu/faractionradar/blob/main/docs/durable-model-card.md",
    }
    ...
    report_path = ROOT / "reports" / year_month / f"durable-t-{days_out}.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_md, encoding="utf-8")
    print(f"[emit-durable] wrote {report_path.relative_to(ROOT)}")
    ...
    print(f"[emit-durable] appended predictions.jsonl")
