...
Sub-models:
  ...
  - FRED MCOILWTICO m/m oil-shock adjustment (~3.5 pts MAE — trend +
    OIL_SHOCK_COEFF * WTI m/m. Coefficient BACKTEST-FIT at -0.068 pts
    per 1% WTI m/m from OLS refit on 24 releases (2024-2026). First
    ship 2026-09-07 used -0.4 which the backtest showed was ~6x too
    large in magnitude; refitted here. Sign confirmed by empirical
    data: oil up -> sentiment down via gas-price transmission.)
...
MAE = {
    "consensus":  1.5,
    "trend":      2.5,
    "oil_shock":  3.5,  # conservative pre-empirical weight
}

# OLS-fit coefficient from backtest run 34099012259 (24 releases). Live
# emit shipped -0.4 initially; backtest showed ~6x magnitude overshoot.
# Refit to -0.068 pts of UMich decline per 1% MCOILWTICO m/m increase.
# Sign consistent with theory (gas-price transmission).
OIL_SHOCK_COEFF = -0.068
...
def fetch_wti_oil_shock(api_key: str, trend_baseline: float | None) -> float | None:
    """Apply WTI m/m adjustment to the trend baseline as an oil-shock
    sub-model estimate. Returns None if either FRED fetch fails or
    trend baseline is unavailable — oil signal is only meaningful as
    modulation on the current sentiment level."""
...
def main() -> None:
    ...
    if oil_shock  is not None: print(f"  oil_shock:  {oil_shock:.1f} (trend + {OIL_SHOCK_COEFF} * WTI m/m)")
